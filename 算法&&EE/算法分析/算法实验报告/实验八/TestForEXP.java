...
public class TestForEXP {
     ...
     * ...
     */
    public static NodeCityMatrix[][] getCityMatri() {
     ...
     * 获取铁路和公路
     */
    public static NodeCity[] getCity() {
...
@Data
class NodeCityMatrix {

    int startPoint;     //起点序号
    String startCity;   //起点城市
    int endPoint;       //终点序号
    String endCity;     //终点城市
    double price;       //航班价格(RMB)
    double time;        //航班时间(day)

    public NodeCityMatrix(int startPoint, String startCity, int endPoint, String endCity, double price, double time) {
...
class NodeCity {
    ...
    int num;                //城市序号
    String name;            //城市名称
    NodeCitySide[] sides;   //邻接城市
    int point;          //指针

    public NodeCity(int num, String name, int size) {
        this.num = num;
        this.name = name;
        sides = new NodeCitySide[size];
        point = 0;
    }

    public void add(NodeCitySide city) {
        sides[point] = city;
        point++;
    }
...
class NodeCitySide {
    ...
    int endPoint;       //终点序号
    String endCity;     //终点城市
    double priceBus;       //公路价格(RMB)
    double timeBus;        //公路时间(day)
    double priceTrain;       //铁路价格(RMB)
    double timeTrain;        //铁路时间(day)

    public NodeCitySide(int startPoint, String startCity, int endPoint, String endCity, double priceBus, double timeBus, double priceTrain, double timeTrain) {
        this.startPoint = startPoint;
        this.startCity = startCity;
        this.endPoint = endPoint;
        this.endCity = endCity;
    ...
    public static Object dfs(NodeCity[] city, String cityName, int budge, List path) {
        NodeCitySide[] sides = null;
        double minPrice = Integer.MAX_VALUE;

        path.add(cityName);         //加入去过的城市

        //得到相邻城市
        for (int i = 1; i < city.length; i++) {
            if (city[i].getName().equals(cityName)) {
                sides = city[i].getSides();
                break;
            }
        }

        // 计算最少价格
        for (NodeCitySide side : sides) {

            boolean flag = path.stream().anyMatch(o -> o.equals(side.getEndCity()));        // 判断要去的城市是否去过。是返回True

            if (side.getPriceBus() < minPrice && !flag) {
                minPrice = side.getPriceBus();
                cityName = side.endCity;
            }
        ...
        if (budge <= 0) {
            ...
            return dfs(city, cityName, budge, path);
        }
    ...
    public static void main(String[] args) {
        ...
        String[] cityList = TestForEXP.getCityList();                   //城市列表
        NodeCity[] city = TestForEXP.getCity();                         //城市间的公路和铁路信息
        NodeCityMatrix[][] cityMatri = TestForEXP.getCityMatri();       //航班信息

        dfs(city, "南京", 5000, new ArrayList());
    }
